...
1. following the template `astro-leetcode/src/content/blog/0-leet-template.md`, **use code committed and pushed in above step** to create a markdown file (name `leet-<4 digit id>-<kebab-title>.md`) to explain the solutions (including time and space complexities). Draw ascii diagrams as needed to help the explanation. If equation is needed, refer to https://katex.org/docs/api. ask the user to review. make sure the constraints section from leetcode is included.
